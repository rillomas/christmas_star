...
fn generate_partial_vertices(
    ...
    short_spike: cgmath::Vector3<f32>) -> Vec<GLfloat> {
...
fn init_buffers(geom : &Geometry) -> (GLuint, GLuint, i32) {
    let mut vao = 0;
    let mut vbo = 0;
...
impl ChristmasStar {
    ...
    pub fn init(&mut self, param: Parameter) -> Result<(), String> {
        ...
        let (vao, vbo, tri_num) = init_buffers(&self.geometry);
